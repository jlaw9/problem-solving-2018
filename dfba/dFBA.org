...
#+BEGIN_SRC ipython :session :exports both :results raw drawer :async t
  ...
  TSPAN = 100
  NegativeCrossing = []
  Lstop=len(P['EX_h2o_LPAREN_e_RPAREN_'])
  i=0
  MaxIter=50
  ...
  while T0 < Tfinal and i < MaxIter:
      ...
      P = ModelDS.compute('test').sample() #simulateModel(ModelDS, ParDef, ICS,TSPAN, T0)
      TSTOP = T0+TSPAN
      AllNegativeValues = {}
      Lstop=len( P['EX_h2o_LPAREN_e_RPAREN_'] )-1
      NegativeCrossing = []
      ...
      if Lstop < len( P['EX_h2o_LPAREN_e_RPAREN_'] )-1:
          B={}
          for variable in P.keys():
...
#+RESULTS:
:RESULTS:
0 - a0ef6338-8255-4917-9fb7-7dc70a90538d
:END:
...
# Out[5]:
: Text(0,0.5,u'gdw')
[[file:./obipy-resources/2290-7Z.png]]
:END:
